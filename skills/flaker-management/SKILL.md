...
- Evidence comes slowly (35 caught real failures to loosen at 0.90). `flaker calibrate --mutate <n>` adds synthetic trials in a temporary worktree; their misses can only tighten, so use it to check the gate is not too loose, never to justify loosening. Preview with `--dry-run`.
