...
- `flaker calibrate --mutate <n> [--seed <n>] [--commits <n>] [--setup <cmd>] [--selector-command <cmd>]` evaluates the selector on synthetic regressions. In a temporary git worktree of HEAD it mutates functions in recently changed source files (flipped comparisons, logical operators and boolean returns, early returns), and per mutation runs the selector and the full suite, storing the record with `source = mutation` and the killed tests as a mutation trial (`flaker_v1.runs.source = mutation`), apart from all real history. Calibration scores mutation records against their trials: a mutation miss tightens and blocks loosening, but mutation kills never count as real failures, so they never loosen. `--dry-run` lists the mutations; `--json` adds `mutation_trials` and `mutation`. `flaker prune` removes old trials (#108).
...
- **One sampling KPI engine.** `status` (health and promotion drift), `plan` / `apply` and `explain bundle`'s `analysis.kpi` now take matched commits, false negative rate, pass correlation, recall, sample ratio and saved minutes from MoonBit `build_sampling_kpi`, the engine `eval` and `run` already used, fed from `flaker_v1.runs` / `results`. The figures are per commit, as the promotion docs describe: a matched commit has both a local sampled run and a CI run with results, and the false negative rate is the share of commits where the local run passed and CI failed. They were per test, joined by suite and test name, so the two parts of one bundle disagreed. The holdout FNR comes from the new MoonBit `build_holdout_kpi` (held-out tests of each commit's latest sampled run that failed in CI on that commit; commits without a CI run are left out). A matched commit now needs local results, not only a recorded sampling run (#108).
...
- Broken / flaky test counts, data confidence, run and result counts, CI-vs-local, and co-failure strength each have one definition (`src/cli/datasets/facts.ts`, over `flaker_v1`), shared by `status`, `plan` / `apply`, `explain bundle` / `context` and `calibrate`:
  - `calibrate`'s intermittent count and `detected_flaky_rate_ratio`, and the KPI's `intermittentFlaky` / `trueFlakyRate` / `flakyTrend`, count `flaker_v1.flaky.is_flaky` tests (with ≥ 5 runs), so tests that fail on some commits without flake evidence no longer raise them.
  - `calibrate` decides CI runs as `flaker_v1.runs.source` does: a run with a local event (`local-import`, `actrun-local`, `flaker-local-run`) no longer counts as CI.
  - `detected_co_failure_strength_ratio` is the mean `flaker_v1.co_failures.strength` of pairs seen on at least 3 commits (it counted result rows, local runs included).
  - `explain context`'s `testCount` counts tests; it counted results. Its `coFailureDataPoints` reads `flaker_v1.co_failures`, within the co-failure window.
  - Every window now ends at "now" (or the injected time), so results dated in the future are not counted.
...
- `flaker calibrate --selector [name]` calibrates the selector's gate from its records and full runs on the same commits. It uses real records only, the latest one per commit, and does not count failures of tests a record quarantined. It replays every record through jev's gate over a grid. On any miss it tightens at once, to a gate that keeps every test the current gate selects and misses the fewest failures. It loosens only with zero misses, at least `min_failures` real failures and a Wilson 95% recall lower bound of at least `recall_target` (at 0.90 that takes 35 real failures). Each run appends one row to `gate_calibration` (`--dry-run` appends nothing). Bare `flaker calibrate` still recommends `[sampling]` and is unchanged.
...
- The TypeScript fallback for stable test ids now produces the same ids as the MoonBit core. It sorted variant keys with `localeCompare` and let JS move integer-like keys to the front, where the core sorts by UTF-16 code unit; lone surrogates in names made the core panic and are now replaced with U+FFFD before either computes an id. Rows written by the fallback with mixed-case (`{B, a}`), non-ASCII (`{é, f}`) or integer-like (`{"10", "9"}`) variant keys, or with lone surrogates in a name, get a different `test_id` after upgrading. The built-in adapters are unaffected: vitest has no variant, playwright uses `{project}`, chaosbringer and vrt-migration use lowercase ASCII keys, and vrt-bench's camelCase keys already sort the same both ways.
- The CLI loads the MoonBit identity core before running a command. It used to load it in the background on the first id, so the first ~300 ms of a run went through the fallback.
