...
[storage]
path = ".flaker/data"      # one DuckDB file
...
| `misses` | The selector's own misses: tests it did not select that really failed in a full run on the same commit, one row per verdict. Only `real` selector runs are scored, against real full runs, and only the latest selector run per `head_sha`, so re-running the selector on a commit does not repeat a miss. A verdict the record itself quarantined (`reason = quarantined`) is not a miss. Mutation records are scored by `calibrate --mutate` instead |
...
`flaker calibrate --selector [name]` joins each real selector record to a full run on its `head_sha`. Only the latest record per `head_sha` counts, so re-running the selector on one commit does not count the same regression twice; mutation records are scored against their trials instead (see below). The ground truth is the tests that failed there, minus flaky and quarantined tests; a failure of a test the record itself quarantined is not the selector's miss and is reported apart. It then replays every record offline through jev's own gate (bundled from `jev-test-filter/gate`, no API calls) over a grid of `cutoff × unsure_below × unsure_margin`. The adoption rule is "tighten at once, loosen with care". If the current gate missed a failure, calibrate switches at once (`tighten`) to a candidate that still selects every test the current gate selects, on every record. Among those it takes the one with the fewest misses, then the fewest selected tests. When no candidate catches every failure, the one with the fewest misses is still adopted if it misses fewer than the current gate. When none misses fewer, the gate is kept and the misses are reported, because selecting more tests would not catch them. Selecting fewer tests (`loosen`) needs zero misses, at least `min_failures` real failures, and a Wilson 95% lower bound of recall of at least `recall_target`. Otherwise the gate is kept, and the reason goes into `rationale` (`keep`). Ties go to the candidate nearest jev's defaults. Each run appends one row to `gate_calibration`; `--dry-run` appends nothing and `--json` prints the result with snake_case keys (`decision.real_failures`, `decision.recall_lb95`, `decision.rationale`, `without_full_run`, `unmatched`). Failures that match no verdict are listed as `unmatched` and not counted as misses, and the report is split by context digest.
...
#### Mutation trials — `flaker calibrate --mutate <n>`

```bash
flaker calibrate --mutate 10 --dry-run             # which mutations it would try
TYPESAFE_API_KEY=… flaker calibrate --mutate 10    # run them, then calibrate
```

Real regressions are rare, so calibration can wait weeks for evidence. `--mutate <n>` makes some: it picks `n` mutations in the JS/TS source files changed by the last `--commits` commits (default 20; tests, declarations and build output are left alone): a flipped comparison (`===` ↔ `!==`, a spaced `<` ↔ `>=`), a flipped logical operator or boolean return, or an early `return;` at the start of a function. `--seed` picks them reproducibly. In a temporary `git worktree` of HEAD (your working tree is never touched; `node_modules` is shared through a symlink, and `--setup <cmd>` runs once there, for example a build) it runs the full suite through `[runner]` on the unmutated tree, then for each mutation commits it, runs the selector against HEAD (`--selector-command`, default `jev-test-filter`, with `--base <HEAD> --context <jev-context> --json`), and runs the full suite again. A test the mutation *killed* is one that failed on the mutated tree and passed on the unmutated one. The selector's record is stored with `source = mutation` and the trial as a full run with `source = mutation` in `flaker_v1.runs`; the kills stay out of `results`, `flaky`, `co_failures` and `misses`, and out of every other command's history. Then the usual `calibrate --selector` runs.

Mutations can only tighten. A kill the selector did not select is a miss like a real one, so it tightens the gate and blocks loosening; but mutation kills never count as `real_failures`, so they never bring a loosening closer. Mutations are distributed differently from real changes, which is why they are kept out of the recall bound. `--json` adds `mutation_trials` (each trial: the mutation, its commit, the record, tests run and killed, or why it was skipped) and `mutation` (`records`, `failures`, `without_trial`, `unmatched`). A trial whose selector fell back (no API key, an API error) writes no record and is skipped without running the suite. `--dry-run` lists the mutations and runs nothing. `flaker prune` removes trials older than its cutoff.
